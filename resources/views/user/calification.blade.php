		...
		<div class="container">
			...
			<div class="bg-white">
				...
				<div class="col-8 offset-2">
					...
					<form action="{{ route('calification.store') }}" enctype="multipart/form-data" method="POST">
						{{csrf_field()}}
				...
				{{-- 	<input type="text" class="form-control"  placeholder="{{$post->url_name}}">
				--}}
...
{{-- 
					...
					<select class="form-control" name="calification_id" id="calification_id">
						@foreach($califications as $calification)
						{{-- <option id="option" value="">Calificar</option> --}}
						<option value="{{$calification->id}}">{{ $calification->calification_name }}</option>
						@endforeach						     
					</select>
					...
					<div class="text-danger" id="msg_email">{{ $errors->first('review') }}</div>
					@endif 

					<textarea name="review" class="form-control editor">
